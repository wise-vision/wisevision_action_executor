...
AutomaticActionService::AutomaticActionService()
    : AutomaticActionService(rclcpp::NodeOptions()) {}

AutomaticActionService::AutomaticActionService(
    const rclcpp::NodeOptions &options)
    : Node("automatic_action_service", options) {
  m_add_automatic_action_service = this->create_service<AutomaticAction>(
      "create_automatic_action",
      std::bind(&AutomaticActionService::handleCreateService, this,
                std::placeholders::_1, std::placeholders::_2));

  m_delete_service = this->create_service<AutomaticActionDelete>(
      "delete_automatic_action",
      std::bind(&AutomaticActionService::handleDeleteService, this,
                std::placeholders::_1, std::placeholders::_2));

  m_connection_service = this->create_service<AutomaticActionConnection>(
      "create_combined_automatic_action",
      std::bind(&AutomaticActionService::handleCombinedService, this,
                std::placeholders::_1, std::placeholders::_2));

  m_available_topics_service = this->create_service<AvailableTopics>(
      "available_topics",
      std::bind(&AutomaticActionService::handleAvailableTopicsService, this,
                std::placeholders::_2));

  m_available_topics_combined_service =
      this->create_service<AvailableTopicsCombined>(
          "available_topics_combined",
          std::bind(
              &AutomaticActionService::handleAvailableTopicsCombinedService,
              this, std::placeholders::_2));

  m_change_automatic_action_service =
      this->create_service<ChangeAutomaticAction>(
          "change_automatic_action",
          std::bind(&AutomaticActionService::handleChangeAutomaticActionService,
                    this, std::placeholders::_1, std::placeholders::_2));

  m_change_automatic_action_combined_service = this->create_service<
      ChangeAutomaticActionCombined>(
      "change_combined_automatic_action",
      std::bind(
          &AutomaticActionService::handleChangeAutomaticActionCombinedService,
          this, std::placeholders::_1, std::placeholders::_2));

  m_combined_delete_service =
      this->create_service<AutomaticActionCombinedDelete>(
          "delete_combined_automatic_action",
          std::bind(&AutomaticActionService::handleCombinedDeleteService, this,
                    std::placeholders::_1, std::placeholders::_2));

  m_notification_publisher =
      this->create_publisher<Notification>("notifications", 10);

  m_yaml_config_manager->loadConfigurationFromFile(
      "config.yaml", m_configurations, m_topics_data);
  initializeSubscriptions();
  m_yaml_config_manager->loadCombinedConfigurationFromFile(
      "config_combined.yaml", m_combined_topics_publishers_and_info);
...
void AutomaticActionService::initializeSubscriptions() {
  for (const auto &[topic, request] : m_configurations) {
    auto captured_request =
        request; // Create a copy of request for safe capture
    auto m_subscription = this->create_generic_subscription(
        request->listen_topic, request->listen_message_type, 10,
        [this,
         captured_request](std::shared_ptr<rclcpp::SerializedMessage> msg) {
...
void AutomaticActionService::initializeCombinedPublishers() {
  for (const auto &[action_and_publisher_name, combined_publisher] :
...
void AutomaticActionService::handleCreateService(
    const std::shared_ptr<AutomaticAction::Request> request,
    std::shared_ptr<AutomaticAction::Response> response) {

  if (request->listen_topic.empty() || request->listen_message_type.empty() ||
      request->action_and_publisher_name.empty() || request->value.empty() ||
      request->trigger_val.empty() || request->trigger_type.empty() ||
      request->publication_method > 6) {
    RCLCPP_WARN(this->get_logger(),
                "Failed to create subscription: some fields in the request are "
                "empty or invalid.");
    response->success = false;
    response->error_message =
        "Some fields in the request are empty or invalid.";
    return;
  }

  if (m_subscriptions.find(request->listen_topic) != m_subscriptions.end()) {
    RCLCPP_WARN(this->get_logger(), "Already monitoring topic: %s",
                request->listen_topic.c_str());
    response->success = false;
    response->error_message =
        "Already monitoring topic: " + request->listen_topic;
    return;
  }

  auto action_and_publisher_name = request->action_and_publisher_name;
  std::shared_ptr<AutomaticActionRequestAndTime> request_and_time =
      std::make_shared<AutomaticActionRequestAndTime>();
  ...
  RCLCPP_INFO(this->get_logger(), "Received request to monitor topic: %s",
              request->listen_topic.c_str());
  ...
  m_yaml_config_manager->saveConfigurationToFile("config.yaml",
                                                 request_and_time);
}

void AutomaticActionService::handleDeleteService(
    const std::shared_ptr<AutomaticActionDelete::Request> request,
    std::shared_ptr<AutomaticActionDelete::Response> response) {
  if (request->listen_topic_to_delete.empty()) {
    RCLCPP_WARN(
        this->get_logger(),
        "Failed to delete subscription: listen_topic_to_delete is empty.");
    response->success = false;
    response->error_message = "listen_topic_to_delete is empty.";
    return;
  }

  for (const auto &[key, combined_topics_publisher] :
       m_combined_topics_publishers_and_info) {
    const auto &topics = combined_topics_publisher->config.listen_topics;
    if (std::find(topics.begin(), topics.end(),
                  request->listen_topic_to_delete) != topics.end()) {
      response->success = false;
      response->error_message =
          "Cannot delete topic that is part of a combined publisher: " +
  ...
  if (topic_to_delete != m_subscriptions.end()) {
    ...
    RCLCPP_INFO(this->get_logger(), "Stopped monitoring topic: %s",
                request->listen_topic_to_delete.c_str());
    response->success = true;
    m_yaml_config_manager->removeSubscriptionFromFile(
        "config.yaml", request->listen_topic_to_delete);
  } else {
    RCLCPP_WARN(this->get_logger(), "Topic not found: %s",
                request->listen_topic_to_delete.c_str());
    response->success = false;
    response->error_message =
        "Topic not found: " + request->listen_topic_to_delete;
  }
}

void AutomaticActionService::replaceExactString(std::string &source,
                                                const std::string &from,
                                                const std::string &to) {
  size_t start_pos = 0;
  while ((start_pos = source.find(from, start_pos)) != std::string::npos) {
    if ((start_pos == 0 || !isalnum(source[start_pos - 1])) &&
        (start_pos + from.length() == source.length() ||
         !isalnum(source[start_pos + from.length()]))) {
...
void AutomaticActionService::handleChangeAutomaticActionService(
  ...
  if (request->action_and_publisher_name_to_change.empty()) {
    RCLCPP_WARN(this->get_logger(),
                "Failed to change action: action_and_publisher_name_to_change "
                "is empty.");
    response->success = false;
  ...
  if (it == m_configurations.end()) {
    RCLCPP_WARN(this->get_logger(),
                "Action with specified listen_topic not found: %s",
                request->listen_topic.c_str());
    response->success = false;
    response->error_message = "Action with specified listen_topic not found: " +
                              request->listen_topic;
    return;
  }

  bool requires_recreation =
      request->new_action_and_publisher_name !=
          it->second->action_and_publisher_name ||
  ...
  if (requires_recreation) {
    ...
    delete_request->listen_topic_to_delete =
        it->second->action_and_publisher_name;
    ...
    if (!delete_response->success) {
      RCLCPP_WARN(this->get_logger(),
                  "Failed to delete action before recreation.");
      response->success = false;
    ...
    create_request->action_and_publisher_name =
        request->new_action_and_publisher_name;
    ...
    if (!create_response->success) {
      RCLCPP_WARN(this->get_logger(),
                  "Failed to create action after deletion.");
      response->success = false;
    ...
    for (auto &[key, combined_publisher] :
         m_combined_topics_publishers_and_info) {
      bool updated = false;
      ...
      for (auto &topic : combined_publisher->config.listen_topics) {
        if (topic == old_action_name) {
          ...
          RCLCPP_INFO(this->get_logger(),
                      "Updated combined topic %s to use new action name %s in "
                      "listen_topics.",
                      key.c_str(), new_action_name.c_str());
        }
      }

      std::string new_logic_expression =
          combined_publisher->config.logic_expression;
      replaceExactString(new_logic_expression, old_action_name,
                         new_action_name);
      std::cout << "New logic expression: " << new_logic_expression
                << std::endl;
      ...
      if (updated) {
        ...
        combined_config.action_and_publisher_name =
            combined_publisher->config.action_and_publisher_name;
        std::cout << "Combined config name: "
                  << combined_config.action_and_publisher_name << std::endl;
        combined_config.listen_topics =
            combined_publisher->config.listen_topics;
        combined_config.logic_expression =
            combined_publisher->config.logic_expression;
        std::cout << "Combined config logic expression: "
                  << combined_config.logic_expression << std::endl;

        auto config_ptr =
            std::make_shared<AutomaticActionCombinedRequestAndTime>(
                combined_config);
        m_yaml_config_manager->modifyCombinedConfigurationInFile(
            "config_combined.yaml", config_ptr);
      }
    }
  ...
  if (m_topics_data.find(request->listen_topic) != m_topics_data.end()) {
    m_topics_data[request->listen_topic]->data_validity_ms =
        request->data_validity_ms;
  }
...
void AutomaticActionService::handleAvailableTopicsService(
    std::shared_ptr<AvailableTopics::Response> response) {
  for (const auto &pair : m_configurations) {
    response->available_topics_with_parameters_and_time.push_back(*pair.second);
  }
}
void AutomaticActionService::handleCombinedService(
    const std::shared_ptr<AutomaticActionConnection::Request> request,
    std::shared_ptr<AutomaticActionConnection::Response> response) {
  if (request->listen_topics.size() < 2 || request->logic_expression.empty() ||
      request->action_and_publisher_name.empty() ||
      request->trigger_text.empty() || request->publication_method > 6) {
    RCLCPP_WARN(this->get_logger(),
                "Failed to create combined publisher: some fields in the "
                "request are empty or invalid.");
    response->success = false;
    response->error_message =
        "Some fields in the request are empty or invalid.";
    return;
  ...
  if (m_combined_topics_publishers_and_info.find(
          request->action_and_publisher_name) !=
      m_combined_topics_publishers_and_info.end()) {
    RCLCPP_INFO(this->get_logger(), "Publisher already exists for topic: %s",
                request->action_and_publisher_name.c_str());
  } else {
    auto publisher = this->create_publisher<std_msgs::msg::String>(
        request->action_and_publisher_name, 10);
    ...
    combined_publisher->publisher = publisher;
    m_combined_topics_publishers_and_info[request->action_and_publisher_name] =
        combined_publisher;
  }
  ...
  m_yaml_config_manager->saveCombinedConfigurationToFile(
      "config_combined.yaml", m_combined_topics_publishers_and_info);
...
void AutomaticActionService::handleCombinedDeleteService(
  ...
  if (request->name_of_combined_topics_publisher.empty()) {
    RCLCPP_WARN(this->get_logger(),
                "Failed to delete combined publisher: "
                "name_of_combined_topics_publisher is empty.");
    response->success = false;
    response->error_message = "name_of_combined_topics_publisher is empty.";
    return;
  }

  auto it = m_combined_topics_publishers_and_info.find(
  ...
  if (it != m_combined_topics_publishers_and_info.end()) {
    ...
    RCLCPP_INFO(this->get_logger(), "Removed combined publisher: %s",
                request->name_of_combined_topics_publisher.c_str());
    response->success = true;

    m_yaml_config_manager->removeCombinedPublisherFromFile(
        "config_combined.yaml", request->name_of_combined_topics_publisher);
  } else {
    RCLCPP_WARN(this->get_logger(), "Combined publisher not found: %s",
                request->name_of_combined_topics_publisher.c_str());
    response->success = false;
  }
}

bool AutomaticActionService::parseExpression(
    const std::string &logic_expression,
      ...
      while (!operator_stack.empty() && operator_stack.top() != "(") {
        if (value_stack.size() < 2) {
          throw std::runtime_error(
              "Invalid expression: not enough values for operation");
        }
  ...
  if (value_stack.size() != 1) {
    throw std::runtime_error(
        "Invalid expression: mismatched operators and values");
  }
...
void AutomaticActionService::messageConnectionCallback() {
  ...
  for (const auto &pair : m_combined_topics_publishers_and_info) {
    ...
    notification_msg->source =
        combined_publisher->config.action_and_publisher_name;
    notification_msg->info = combined_publisher->config.trigger_text;
    notification_msg->publication_method =
        combined_publisher->config.publication_method;
    ...
    for (const auto &topic : combined_publisher->config.listen_topics) {
      ...
      if (it == m_topics_data.end()) {
        RCLCPP_ERROR(this->get_logger(), "Topic data for %s not found",
                     topic.c_str());
      ...
      int64_t elapsed_time_ms =
          (this->now().nanoseconds() - topic_data->time_stamp.nanoseconds()) /
          1000000;
      bool is_valid = topic_data->action_triggered &&
                      (elapsed_time_ms <= topic_data->data_validity_ms);
      topic_values[topic] = is_valid;
    }

    bool expression_result = parseExpression(
        combined_publisher->config.logic_expression, topic_values);
    ...
    if (expression_result) {
      ...
      RCLCPP_INFO(this->get_logger(), "Published message to %s",
...
void AutomaticActionService::handleChangeAutomaticActionCombinedService(
  ...
  auto it = m_combined_topics_publishers_and_info.find(
      request->action_and_publisher_name_to_change);
  if (it == m_combined_topics_publishers_and_info.end()) {
    RCLCPP_WARN(this->get_logger(), "Combined publisher not found: %s",
                request->action_and_publisher_name_to_change.c_str());
    response->success = false;
    response->error_message = "Combined publisher not found: " +
                              request->action_and_publisher_name_to_change;
    return;
  }

  bool needs_recreation = request->new_action_and_publisher_name !=
                          request->action_and_publisher_name_to_change;

  bool all_topics_exist = std::all_of(
      request->listen_topics.begin(), request->listen_topics.end(),
  ...
  if (!all_topics_exist) {
    RCLCPP_WARN(
        this->get_logger(),
        "One or more topics in listen_topics do not exist in configurations.");
    response->success = false;
    response->error_message =
        "One or more topics in listen_topics do not exist in configurations.";
    return;
  ...
  if (needs_recreation) {
    auto delete_request =
        std::make_shared<AutomaticActionCombinedDelete::Request>();
    delete_request->name_of_combined_topics_publisher =
        request->action_and_publisher_name_to_change;
    auto delete_response =
        std::make_shared<AutomaticActionCombinedDelete::Response>();
    handleCombinedDeleteService(delete_request, delete_response);

    if (!delete_response->success) {
      RCLCPP_WARN(this->get_logger(),
                  "Failed to delete old combined publisher before recreation.");
      response->success = false;
      response->error_message =
          "Failed to delete old combined publisher before recreation.";
      return;
    ...
    auto create_request =
        std::make_shared<AutomaticActionConnection::Request>();
    ...
    auto create_response =
        std::make_shared<AutomaticActionConnection::Response>();
    ...
    if (request->publication_method !=
        combined_publisher->config.publication_method) {
      combined_publisher->config.publication_method =
          request->publication_method;
    }

    auto combined_publisher_convert =
        std::make_shared<AutomaticActionCombinedRequestAndTime>();
    ...
    combined_publisher_convert->action_and_publisher_name =
        request->new_action_and_publisher_name;
    combined_publisher_convert->trigger_text = request->trigger_text;
    combined_publisher_convert->publication_method =
        request->publication_method;

    m_yaml_config_manager->modifyCombinedConfigurationInFile(
        "config_combined.yaml", combined_publisher_convert);
...
void AutomaticActionService::handleAvailableTopicsCombinedService(
  ...
  for (const auto &pair : m_combined_topics_publishers_and_info) {
    response->available_combined_topics_with_parameters_and_time.push_back(
        pair.second->config);
  }
}

void AutomaticActionService::messageCallback(
    const std::shared_ptr<rclcpp::SerializedMessage> msg,
    const std::shared_ptr<AutomaticActionRequestAndTime> config) {
  auto introspection_ts = m_value_extractor.getIntrospectionTypeSupport(
      config->listen_message_type);
  ...
  auto value =
      m_value_extractor.extractValue(*msg, introspection_ts, config->value);
  if (!value.has_value()) {
    RCLCPP_ERROR(this->get_logger(), "Failed to extract value for field: %s",
                 config->value.c_str());
    return;
  }

  std::variant<double, float, int8_t, int16_t, int32_t, int64_t, uint8_t,
               uint16_t, uint32_t, uint64_t, bool, char, std::string>
      field_value;
  ...
  try {
    if (auto v = std::any_cast<double>(&*value)) {
      ...
      RCLCPP_ERROR(this->get_logger(), "Unsupported type for field: %s",
                   config->value.c_str());
      return;
    }
      ...
      [&](auto &&arg) {
        ...
        if constexpr (std::is_arithmetic_v<T>) {
          if (config->trigger_type ==
              AutomaticActionService::LESS_THAN_TRIGGER) {
            trigger_condition = arg < static_cast<T>(trigger_val);
          } else if (config->trigger_type ==
                     AutomaticActionService::GREATER_THAN_TRIGGER) {
            trigger_condition = arg > static_cast<T>(trigger_val);
          } else if (config->trigger_type ==
                     AutomaticActionService::EQUAL_TO_TRIGGER) {
            trigger_condition = arg == static_cast<T>(trigger_val);
          } else {
            RCLCPP_ERROR(this->get_logger(), "Unsupported trigger type: %s",
                         config->trigger_type.c_str());
          }
          ...
          if (config->trigger_type ==
            ...
            RCLCPP_ERROR(
                this->get_logger(),
                "String comparisons are only supported with 'EqualTo'");
          }
        } else {
          RCLCPP_ERROR(this->get_logger(), "Unsupported type for field: %s",
                       config->trigger_type.c_str());
  ...
  if (trigger_condition) {
    ...
    if (it == m_configurations.end()) {
      ...
      std::cout << "Date and time: " << it->second->date_and_time.year << "-"
                << it->second->date_and_time.month << "-"
                << it->second->date_and_time.day << " "
                << it->second->date_and_time.hour << ":"
                << it->second->date_and_time.minute << ":"
                << it->second->date_and_time.second << "."
                << it->second->date_and_time.nanosecond << std::endl;
      m_yaml_config_manager->modifyConfigurationInFile("config.yaml",
                                                       it->second);
    }
...
void AutomaticActionService::performAction(
    const std::shared_ptr<AutomaticActionRequestAndTime> config) {
  auto pub_msg = std::make_shared<std_msgs::msg::String>();
  ...
  if (m_publishers.find(config->action_and_publisher_name) ==
      m_publishers.end()) {
    auto publisher = this->create_publisher<std_msgs::msg::String>(
        config->action_and_publisher_name, 10);
...
RCLCPP_COMPONENTS_REGISTER_NODE(AutomaticActionService)